...
def solution(jobs):
    ...
    while jobs:
        readyQueue = [[idx, process[0],process[1]] for idx,process in enumerate(jobs) if process[0]<=current_time]
        
        if len(readyQueue) == 0 : 
            current_time += 1
            continue

        idx, receive_time, need_time = min(readyQueue, key = lambda n:(n[2],-n[1]))
        answer += current_time - receive_time + need_time
        current_time += jobs.pop(idx)[1]
                
    return int(answer / count)
```


참고 : <https://programmers.co.kr/learn/courses/30/lessons/42627>